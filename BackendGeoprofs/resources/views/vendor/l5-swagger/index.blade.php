@php
    // Provide safe defaults so this published view works with older or newer l5-swagger versions
    ...
    if (isset($documentations[$documentation])) {
        $docConfig = $documentations[$documentation]['api'] ?? [];
    } elseif (!empty($documentations)) {
        $first = reset($documentations);
        $docConfig = $first['api'] ?? [];
    }
    $documentationTitle = $documentationTitle ?? ($docConfig['title'] ?? config('app.name'));
